...
while (($timezone_diff = (gmmktime() - time()))%10);
...
while (!feof($handle)) {
	...
	$dbRecord['date']=date('Y-m-d',$record[0]+$timezone_diff);
	$dbRecord['time']=date('H:i:s',$record[0]+$timezone_diff);
	$dbRecord['summaryTime']=date('H',$record[0]+$timezone_diff);
	...
	$query='INSERT INTO traffic(date,time,ip,resultCode,bytes,url,authuser) VALUES (';
		$query.="'".$dbRecord['date']."'";
		$query.=',';
		$query.="'".$dbRecord['time']."'";
		$query.=',';
